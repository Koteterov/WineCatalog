...
`)(t))})),xe("/catalog",(async function(e){e.render(z(async function(e){const t=e.user,n=Number(e.query.page)||1;try{const{data:e,pages:i}=await async function(e){let t="/data/wines?pageSize=3&offset="+3*(e-1);const[n,i]=await Promise.all([ke(Se+t),ke(Se+"/data/wines?count")]);return{data:n,pages:Math.ceil(i/3)}}(n);return((e,t,n,i)=>V`
...
`)(e,t,n,i)}catch(e){console.log(e)}}(e),V`<h1 id="loading">Loading...</h1>`))})),xe("/login",(async function(e){e.render(V`
        <section id="loginPage">
            <form @submit=${async function(t){t.preventDefault();const n=new FormData(t.target),i=n.get("email").trim(),s=n.get("password").trim();if(i&&s)try{await async function(e,t){const n=await Pe($e.host+"/users/login",{email:e,password:t});return sessionStorage.setItem("email",n.email),sessionStorage.setItem("authToken",n.accessToken),sessionStorage.setItem("userId",n._id),n}(i,s),e.setUserNav(),e.page.redirect("/")}catch(e){Me(e.message)}else Me("Please fill in both fields!")}}>
                <fieldset>
                    <legend>Login</legend>
  ...
  <section id="detailsPage">
    ...
    ${u?V`
          <article class="create-comment">
            ...
            <form @submit=${async function(t){t.preventDefault();const i=new FormData(t.target).get("comment").trim();try{if(""==i)throw new Error("Please fill in !");await async function(e){return await Pe(Se+Ce,e)}({wineId:n,comment:i}),t.target.reset(),e.page.redirect(`/details/${n}`)}catch(e){Me(e.message)}}} class="form">
...
`)(i,u,0,d,0,s,r,0,h,0,p))})),xe("/create",(async function(e){e.render(V`
  <section class="createPage">
    <form @submit=${async function(t){t.preventDefault();const n=new FormData(t.target),i=n.get("name").trim(),s=n.get("imgUrl").trim(),a=n.get("price").trim(),r=n.get("netQuantity").trim(),o=n.get("origin").trim(),l=n.get("supplier").trim(),c=n.get("storage").trim();try{if(!(i&&s&&a&&r&&o&&l&&c))throw new Error("Please fill in all fields!");await async function(e){return await Pe(Se+Te,e)}({name:i,imgUrl:s,price:a,netQty:r,origin:o,supplier:l,storage:c}),t.target.reset(),e.page.redirect("/catalog")}catch(e){Me(e.message)}}}>
      <fieldset>
...
`)})),xe("/edit/:id",(async function(e){const t=e.params.id,n=await Le(t);e.render(((n,i)=>V`
  <section class="editPage">
    <form @submit=${async function(n){n.preventDefault();const i=new FormData(n.target),s=i.get("name").trim(),a=i.get("imgUrl").trim(),r=i.get("price").trim(),o=i.get("netQuantity").trim(),l=i.get("origin").trim(),c=i.get("supplier").trim(),d=i.get("storage").trim();try{if(!(s&&a&&r&&o&&l&&c&&d))throw new Error("Please fill in all fields!");await async function(e,t){return await async function(e,t){return await Ne(e,Ee("put",t))}(Se+Ue(e),t)}(t,{name:s,imgUrl:a,price:r,netQty:o,origin:l,supplier:c,storage:d}),n.target.reset(),e.page.redirect("/catalog")}catch(e){Me(e.message)}}}>
      <fieldset>
...
`)(n))})),xe("/mywines",(async function(e){const t=e.user;try{const i=await async function(e){return await ke(Se+(e=>`/data/wines?where=_ownerId%3D%22${e}%22&sortBy=_createdOn%20desc`)(e))}(t);e.render(V`
  <section id="catalogPage">
    <h1>My Wines</h1>
...
`)}catch(e){console.log(e)}var n})),xe("/search",(async function(e){const t=e.query.query,n=null==t?[]:await async function(e){return await ke(Se+(e=>`/data/wines?where=name%20LIKE%20%22${e}%22`)(e))}(t),i=e.user;e.render(((t,n,i,s)=>V`
    ...
    <form>
      <div class="search">
        <input
          ...
          .value=${i||""}
        />
        <button @click=${function(){const t=document.getElementById("search-input");if(""==t.value)return void Me("Please fill in!");e.page.redirect(`search?query=${t.value.trim()}`)}} class="button-list">Search</button>
      </div>
    </form>
      ...
      ${n.length>0?q(n,(e=>e._id),(e=>V`
              <div class="card-box">
                ...
                <div>
                  ...
                  <div class="btn-group">
                    ${s?V`
                          <a href="/details/${e._id}" id="details"
                            >Details</a
                          >
                        `:o}
                  </div>
